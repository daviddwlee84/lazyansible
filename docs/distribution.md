...
Version **v0.1.0** provides macOS and Linux binaries for amd64 and arm64. Go is
...
For standalone installs, download the matching `lazyansible_VERSION_OS_ARCH.tar.gz`
from [GitHub Releases](https://github.com/daviddwlee84/lazyansible/releases), verify its
...
```sh
go install github.com/daviddwlee84/lazyansible/cmd/lazyansible@v0.1.0
# Update an existing Go installation through its original destination:
go install github.com/daviddwlee84/lazyansible/cmd/lazyansible@latest
```
...
Source tags recover their module version; release archives inject v0.1.0; local
...
python3 scripts/release.py check --project lazyansible --binary lazyansible --smoke
