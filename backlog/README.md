# Backlog research

[TODO.md](../TODO.md) is the index of future work. These notes preserve context,
tradeoffs, evidence, and the next question to resolve. Add a note for `P?`, `L` /
`XL`, meaningful paused investigation, or a choice with multiple viable options.
Small obvious tasks need only a TODO entry. Completed troubleshooting belongs in
[pitfalls/](../pitfalls/README.md); current usage belongs in the main README.

| Topic | Status | Next decision |
| --- | --- | --- |
| [Release and upgrade](release-and-upgrade.md) | P2 / L | Set supported installation channels after the local trial |
| [Variable provenance and graphs](variable-provenance-graphs.md) | P? / L | Choose bounded static structure versus instrumented execution evidence |

[Inbox](inbox.md) accepts ideas whose priority or effort is not clear yet. These
files are maintainer metadata retained in source; the binary archives do not
include them.
